...
import static org.beyondcoinj.core.NetworkParameters.MAX_MONEY;
import static org.beyondcoinj.utils.BtcAutoFormat.Style.CODE;
import static org.beyondcoinj.utils.BtcAutoFormat.Style.SYMBOL;
import static org.beyondcoinj.utils.BtcFixedFormat.REPEATING_DOUBLETS;
import static org.beyondcoinj.utils.BtcFixedFormat.REPEATING_TRIPLETS;
...
public class BtcFormatTest {
    ...
    public void parseTest() throws java.text.ParseException {
        ...
        assertEquals(valueOf(200000000), us.parseObject("2"));
        assertEquals(valueOf(200000000), usCoded.parseObject("BYND 2"));
    ...
    public void parseMetricTest() throws ParseException {
        ...
        // coin
        assertEquals(parseCoin("1"), cp.parseObject("BYND 1.00"));
    ...
    public void currencyCodeTest() {
        ...
        assertEquals("BYND 1.00", usCoded.format(COIN));
        ...
        assertEquals("1,00 BYND", frCoded.format(COIN));
        ...
        assertEquals("BYND 1,00", deCoded.format(COIN));
    }
    ...
    public void autoDecimalTest() {
        ...
        assertEquals("BYND 1", codedZero.format(COIN));
        ...
        assertEquals("BYND 1,000", codedZero.format(COIN.multiply(1000)));
        ...
        assertEquals("BYND 1.00", codedTwo.format(COIN));
        ...
        assertEquals("BYND 1,000.00", codedTwo.format(COIN.multiply(1000)));
        ...
        assertEquals("BYND 1.000", codedThree.format(COIN));
        ...
        assertEquals("BYND 1,000.000", codedThree.format(COIN.multiply(1000)));
    ...
    public void symbolsCodesTest() {
        ...
        BtcFixedFormat symbolCoin = (BtcFixedFormat)BtcFormat.builder().locale(US).scale(0).
                                                              symbol("B\u20e6").build();
        assertEquals("BYND", symbolCoin.code());
    ...
    public void negativeTest() throws Exception {
        assertEquals("-1,00 BYND", BtcFormat.getInstance(FRANCE).format(COIN.multiply(-1)));
        assertEquals("BYND -1,00", BtcFormat.getInstance(ITALY).format(COIN.multiply(-1)));
        assertEquals("฿ -1,00", BtcFormat.getSymbolInstance(ITALY).format(COIN.multiply(-1)));
        assertEquals("BYND -1.00", BtcFormat.getInstance(JAPAN).format(COIN.multiply(-1)));
        assertEquals("฿-1.00", BtcFormat.getSymbolInstance(JAPAN).format(COIN.multiply(-1)));
        assertEquals("(BYND 1.00)", BtcFormat.getInstance(US).format(COIN.multiply(-1)));
        assertEquals("(฿1.00)", BtcFormat.getSymbolInstance(US).format(COIN.multiply(-1)));
        // assertEquals("BYND -१.००", BtcFormat.getInstance(Locale.forLanguageTag("hi-IN")).format(COIN.multiply(-1)));
        assertEquals("BYND -๑.๐๐", BtcFormat.getInstance(new Locale("th","TH","TH")).format(COIN.multiply(-1)));
    ...
    public void attributeTest() throws Exception {
        ...
        assertTrue(codePat.contains("BYND") && ! codePat.contains("(^|[^฿])฿([^฿]|$)") && ! codePat.contains("(^|[^¤])¤([^¤]|$)"));
        String symPat = BtcFormat.getSymbolInstance(Locale.US).pattern();
        assertTrue(symPat.contains("฿") && !symPat.contains("BYND") && !symPat.contains("¤¤"));

        assertEquals("BYND #,##0.00;(BYND #,##0.00)", BtcFormat.getCodeInstance(Locale.US).pattern());
    ...
    public void toStringTest() {
        ...
        assertEquals("Auto-format BYND #,##0.00;(BYND #,##0.00)", BtcFormat.getCodeInstance(Locale.US).toString());
        assertEquals("Auto-format BYND #,##0.0000;(BYND #,##0.0000)", BtcFormat.getCodeInstance(Locale.US, 4).toString());
    ...
    public void patternDecimalPlaces() {
        ...
        assertEquals("Millicoin-format BYND #.000;BYND -#.000", f.toString());
    ...
    public void builderTest() {
        ...
        assertEquals("BYND -1,00", built.format(COIN.multiply(-1)));
